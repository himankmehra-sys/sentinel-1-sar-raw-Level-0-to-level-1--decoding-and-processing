...
''' ---------------------------- Refer to example_read_processing_radar_data.py to run ---------------------------- '''
# enter saved dictionary address, which was stored after running read_sentinel_data.py
stored_address_and_filename = joblib.load('...\\stored_address_and_filename.joblib')
...
for file in tqdm(chunk_files):
    ...
    if len(np.load(f'{stored_address_and_filename['save_chunks_path_address']}\\{file}')) != 0:
        ...
        tiles_raw_iq_array = tiles_of_data(radar_data=radar_data,
                                           start_limit=0,
                                           end_limit=len(radar_data))
